...
from .pokemon import (get_random_move, get_species_id_by_label, randomize_abilities, randomize_base_stats,
                      randomize_evolutions, randomize_learnsets, randomize_legendary_encounters,
                      randomize_misc_pokemon, randomize_starters, randomize_tm_hm_compatibility, randomize_types,
...
class PokemonEmeraldWorld(World):
    ...
    def generate_early(self) -> None:
        ...
        if "Fly Without Badge" in self.options.hm_requirements.value:
            self.hm_requirements["HM02 Fly"] = 0
        if "Surf Requires Extra Badge" in self.options.hm_requirements.value:
            if self.options.badges != RandomizeBadges.option_completely_random:
                raise OptionError(f"Pokemon Emerald: Player {self.player} ({self.player_name}) can't enable \"Surf "
                                   "Requires Extra Badge\" unless their Randomize Badges option is set to Completely "
                                   "Random.")
        ...
        if self.options.badges != RandomizeBadges.option_completely_random:
            if any(
                ...
                for req in self.options.custom_hm_requirements.value.values()
            ):
                raise OptionError(f"Pokemon Emerald: Player {self.player} ({self.player_name}) cannot specify custom "
    ...
    def create_items(self) -> None:
        ...
        if self.options.item_pool_type == ItemPoolType.option_shuffled:
            ...
            item_category_counter: Counter[str] = Counter()
            ...
            fill_item_candidates_by_category: dict[str, list[int]] = {category: [] for category in item_categories}
